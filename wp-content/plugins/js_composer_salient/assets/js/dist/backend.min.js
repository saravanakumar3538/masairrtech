    ...
    function($) {
        ...
        vc.edit_form_callbacks = [], vc.atts = {
                ...
                $(".vc_link-build", $field).click(function(e) {
                    ...
                    jQuery(document).off("wplink-open", onOpen).on("wplink-open", onOpen), jQuery(document).off("wplink-close", onClose).on("wplink-close", onClose), dialog.open("content"), _.isString(value_object.url) && ($("#wp-link-url").length ? $("#wp-link-url").val(value_object.url) : $("#url-field").val(value_object.url)), _.isString(value_object.title) && ($("#wp-link-text").length ? $("#wp-link-text").val(value_object.title) : $("#link-title-field").val(value_object.title)), $("#wp-link-target").length ? $("#wp-link-target").prop("checked", !_.isEmpty(value_object.target)) : $("#link-target-checkbox").prop("checked", !_.isEmpty(value_object.target)), $("#vc-link-nofollow").length && $("#vc-link-nofollow").prop("checked", !_.isEmpty(value_object.rel)), $vc_link_submit.unbind("click.vcLink").bind("click.vcLink", function(e) {
                        ...
                        return options.target = $checkbox[0].checked ? " _blank" : "", options.rel = $("#vc-link-nofollow")[0].checked ? "nofollow" : "", string = _.map(options, function(value, key) {
                            if (_.isString(value) && 0 < value.length) return key + ":" + encodeURIComponent(value)
                        }).join("|"), $input.val(string), $input.data("json", options), $url_label.html(options.url + options.target), $title_label.html(options.title), dialog.close(), $link_submit.show(), $vc_link_submit.unbind("click.vcLink"), $vc_link_submit.remove(), 
                        $("#wp-link-cancel").unbind("click.vcLink"), window.wpLink.textarea = "", $checkbox.attr("checked", !1), $("#most-recent-results").css("top", ""), $("#vc-link-nofollow").attr("checked", !1), !1
            ...
            init: function(param, $field) {
                var content = $field,
                    $field_input = $(".wpb_vc_param_value[name=" + param.param_name + "]", content);
                $('[data-vc-action="open"]', content).click(function(e) {
                    e.preventDefault();
                    var $frame, gutenbergParamObj, iframeURL, value = $field_input.val();
                    ...
                    var closeEditor = function(e) {
                        e && e.preventDefault && e.preventDefault(), gutenbergParamObj && gutenbergParamObj.updateValueFromIframe(), _.delay(function() {
                            content.find(".vc_gutenberg-modal-wrapper").html(""), gutenbergParamObj = $frame = null
                        }, 100)
                    };
                    gutenbergParamObj = new GutenbergParam({
                        onSetValue: function(value) {
                            $field_input.val(value)
                        },
                        onError: closeEditor,
                        value: value
                    }), vc.createOverlaySpinner(), content.find(".vc_gutenberg-modal-wrapper").html('<div class="wpb-gutenberg-modal"><div class="wpb-gutenberg-modal-inner"><button class="wpb-gutenberg-modal-close-button"><i class="vc-composer-icon vc-c-icon-close"/></button><iframe style="width: 100%;" data-vc-gutenberg-param-iframe></iframe></div></div>'), $frame = content.find("iframe[data-vc-gutenberg-param-iframe]"), content.find(".wpb-gutenberg-modal-close-button").click(closeEditor), $frame.attr("src", iframeURL), $frame.load(function() {
                        vc.removeOverlaySpinner(), gutenbergParamObj && (gutenbergParamObj.iframe = $frame.get(0), gutenbergParamObj.iframeLoaded())
                    })
                })
            }
        };
        var GutenbergParam = function(options) {
            return this.iframe = null, this.options = options || {}, this.value = this.options && this.options.value ? this.options.value : "", this.iframeLoaded = function() {
                var localizations, alertNotice, wpData, debounce, editor, newPost, editorSettings;
                (wpData = !!this.iframe.contentWindow.wp && this.iframe.contentWindow.wp.data) || (alertNotice = (localizations = !!window.i18nLocale && window.i18nLocale) && localizations.gutenbergDoesntWorkProperly ? localizations.gutenbergDoesntWorkProperly : "Gutenberg plugin doesn't work properly. Please check Gutenberg plugin.", window.alert(alertNotice), this.options && this.options.onError && this.options.onError()), debounce = _.debounce(this.updateValueFromIframe.bind(this), 500), this.iframe.contentWindow._wpGutenbergPost && (wpData.subscribe(debounce), editorSettings = {
                    ...
                    disablePostFormats: !1,
                    titlePlaceholder: "",
                    bodyPlaceholder: "Add content to apply to WPB ;)"
                }, (newPost = Object.assign({}, this.iframe.contentWindow._wpGutenbergPost)).content.raw = this.value, newPost.content.rendered = this.value, editor = wpData.dispatch("core/editor"), wpData.select("core/edit-post").isPublishSidebarOpened = function() {
                    return !0
                }, "function" == typeof editor.autosave && (editor.autosave = function() {}), editor.setupEditor(newPost, editorSettings))
            }, this.updateValueFromIframe = function() {
                var value;
                this.iframe && this.iframe.contentWindow && this.iframe.contentWindow.wp && this.iframe.contentWindow.wp.data && (value = this.iframe.contentWindow.wp.data.select("core/editor").getEditedPostContent(), this.setValue(value))
        ...
        vc.clone_index = 1, vc.shortcode_view = Backbone.View.extend({
            ...
            changeShortcodeParams: function(model) {
                ...
                tag = model.get("shortcode"), params = model.get("params"), settings = vc.map[tag], _.defer(function() {
                    ...
                    }).join(", ")) : _.isObject(param_settings.value) && !_.isArray(param_settings.value) && (inverted_value = _.invert(param_settings.value), label_value = _.isString(inverted_value[value]) ? inverted_value[value] : value), 
                    $admin_label.html("<label>" + $admin_label.find("label").text() + "</label>: " + label_value), $admin_label.show().removeClass("hidden-label")))
